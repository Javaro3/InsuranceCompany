...
namespace Service {
    public class InsuranceCompanyFilter {
        ...
        public IEnumerable<InsuranceType> Filter(InsuranceTypeFilterModel filter) {
            ...
            if (filter != null) {
                return result
                    .Where(e => filter.Name == null || e.Name.StartsWith(filter.Name))
                    .Where(e => filter.Description == null || e.Description.StartsWith(filter.Description))
                    .Sort(filter);
            }
            return result;
        }
        ...
        public IEnumerable<Contract> Filter(ContractFilterModel filter) {
            ...
            if (filter != null) {
                return result
                    .Where(e => filter.Responsibilities == null || e.Responsibilities.StartsWith(filter.Responsibilities))
